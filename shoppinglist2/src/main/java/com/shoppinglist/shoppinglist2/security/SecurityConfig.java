...
//        http.csrf(AbstractHttpConfigurer::disable)  // CSRF 보호 비활성화(Stateless JWT사용
//                .cors(Customizer.withDefaults())    // CORS 서정(이하의 설정 사용)
//                .sessionManagement(session -> session.sessionCreationPolicy(SessionCreationPolicy.STATELESS))  // 세션 비활성화
//                .authorizeHttpRequests(auth -> auth
//                        // login 엔드포인트의 POST 요쳥은
//                        .requestMatchers(HttpMethod.POST, "/login").permitAll()
//                        .requestMatchers("/swagger-ui/**", "/api-docs/**").permitAll()
//                        .anyRequest().authenticated())
//                        .exceptionHandling(ex -> ex.authenticationEntryPoint(authEntryPoint))  // 인증 처리시 실ㅠㅐ
